...
public class AuthorityServiceImpl implements AuthorityService {
    ...
    public void checkBasicRoles(User admin) {
        ...
        // admin账号必须存在与admin组
        if (0 == dao.count("t_user_role", Cnd.where("user_id", "=", admin.getId()).and("role_id", "=", adminRole.getId()))) {
        ...
        OUT:
        for (Permission permission : dao.query(Permission.class, Cnd.where("name", "like", "authority:%").or("name", "like", "user:%"), null)) {
            for (Record re : res) {
